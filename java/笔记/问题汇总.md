...
1. `java.lang.IllegalStateException: 启动子级时出错 ... ... 名为 [xxx]和 [xxx] 的servlet不能映射为一个url模式(url-pattern) [/ServletA]`:该问题多为注解与`web.xml`配置同时设置发生冲突导致，删去其中一个即可解决
...
git config --global --edit  # 查看和修改git的配置项
# 为git配置代理
