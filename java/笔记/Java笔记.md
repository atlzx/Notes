...
|^|`volatile`|标记该变量在多线程环境下可见且不会被指令重排|无|
...
|`wait()`|无参数|使线程释放锁，并进入等待状态|无返回值|`void`|^|^|
|`wait(long timeoutMillis)`|`timeoutMillis`:指定线程等待的最大毫秒数|使线程释放锁，并进入等待状态，线程最多等待`timeoutMillis毫秒`时长后自动被唤醒|无返回值|`void`|^|^|
|`wait(long timeoutMillis,int nanos)`|`timeoutMillis`:指定线程等待的最大毫秒数<br>`nanos`:指定线程等待的最大纳秒数|线程释放锁，并进入等待状态，线程最多等待`timeoutMillis毫秒+nanos纳秒`时长后自动被唤醒|无返回值|`void`|^|^|
...
~~~java
    ...
    final V putVal(int hash, K key, V value, boolean onlyIfAbsent,
        ...
        // (n-1)&hash表示将key的哈希值与(当前数组长度-1)进行与运算,然后得到结果(实际上就是对着数组长度取模，只有当n的值为2的幂时，该与运算才会与取模运算得到的结果相同)，用位运算是因为效率快
...
+ 在`java.lang.Thread.State`的枚举类中这样定义:
  ...
  + 在新的生命周期内，**阻塞被拆分为了三种状态**:
    + 4.`BLOCKED（锁阻塞）`:正在等待获得锁的已经开始运行的线程处于该状态
      + 例:线程A和B使用同一个锁，如果A获得锁，那么B进入该状态
    + 5.`TIMED_WAITING（计时等待）`:线程在进入该状态后，若在一定时间内（可以自行设置）没有其它线程来唤醒，那么自己就自动被唤醒
      + 线程执行时碰到明确设置了时间的`Thread`类的`sleep`、`join`方法，或`Object`类的`wait`方法，或`LockSupport`的`park`方法后，处于该状态
    + 6.`WAITING（无限等待）`:如果没有其它线程来唤醒该线程，那么线程就会一直等下去
...
+ [样例](../../java/源码/JUC/BasicJUC/src/main/java/DaemonAndUserThread.java)
...
+ **同步代码块**通过使用`synchronized`关键字修饰代码块来使该段代码被同步执行
  + 格式:`synchronized(同步锁/同步监视器){...}`
    ...
    + `同步锁/同步监视器`表示的就是synchronized代码块内的代码要执行时所需要占用的对象，也就是锁。一旦该对象被某一线程占用，其它线程就只能等待该线程的同步代码块执行完毕后释放对该对象的占用，也就是释放锁。
